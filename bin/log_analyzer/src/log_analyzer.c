...
int main(int argc, char ** argv){
	...
	job_finish_times.array 			= malloc(ARRAY_SIZE * sizeof(uint64_t));
	launched_jobs.array 			= malloc(ARRAY_SIZE * sizeof(uint64_t));
	queued_jobs.array 				= malloc(ARRAY_SIZE * sizeof(uint64_t));
	finished_jobs.array 			= malloc(ARRAY_SIZE * sizeof(uint64_t));
	aborted_jobs.array 				= malloc(ARRAY_SIZE * sizeof(uint64_t));
	jobs_times.array 				= malloc(ARRAY_SIZE * sizeof(uint64_t));
	cost_per_second.array 			= malloc(ARRAY_SIZE * sizeof(uint64_t));
	cpu_usage.array 				= malloc(ARRAY_SIZE * sizeof(uint64_t));
	mem_usage.array 				= malloc(ARRAY_SIZE * sizeof(uint64_t));
	nodes_usage.array 				= malloc(ARRAY_SIZE * sizeof(uint64_t));
	diff_submit_start_jobs.array 	= malloc(ARRAY_SIZE * sizeof(uint64_t));
	...
	if ((line_buffer = calloc(LINE_BUFFER, sizeof(char))) == NULL) {
		terror("Could not allocate memory for line buffer");
	}
	if ((header_buffer = calloc(LINE_BUFFER, sizeof(char))) == NULL) {
		terror("Could not allocate memory for header buffer");
	}
	if ((job_launched_aux_string = calloc(LINE_BUFFER, sizeof(char))) == NULL) {
		terror("Could not allocate memory for job_launched_aux_string");
	}
	if ((output_namefile = calloc(LINE_BUFFER, sizeof(char))) == NULL) {
		terror("Could not allocate memory for output_namefile");
	}
	if ((aux = calloc(LINE_BUFFER, sizeof(char))) == NULL) {
		terror("Could not allocate memory for output_namefile");
	}
	...
	while (fgets(line_buffer, LINE_BUFFER, input_file)) {
		...
		if (!strcmp(header_buffer, "[JOB FINISHED]")) {
			if (job_finish_times.number_of_reallocs * ARRAY_SIZE == job_finish_times.i) {
				...
				job_finish_times.array = realloc(job_finish_times.array, 
			...
			if (diff_submit_start_jobs.number_of_reallocs * ARRAY_SIZE == diff_submit_start_jobs.i) {
				...
				diff_submit_start_jobs.array = realloc(diff_submit_start_jobs.array, 
			...
			if (!strcmp(header_buffer, "[SYSTEM OFF]")) {
				...
				if (!strcmp(header_buffer, "[SYS LOAD]")){
					...
					jobs_times.array = realloc(jobs_times.array, jobs_times.number_of_reallocs * ARRAY_SIZE * sizeof(uint64_t));
					if (jobs_times.array == NULL) terror("Could not reallocate jobs_times array");
					queued_jobs.array = realloc(queued_jobs.array, jobs_times.number_of_reallocs * ARRAY_SIZE * sizeof(uint64_t));
					if (queued_jobs.array == NULL) terror("Could not reallocate queued_jobs array");
					launched_jobs.array= realloc(launched_jobs.array, jobs_times.number_of_reallocs * ARRAY_SIZE * sizeof(uint64_t));
					if (launched_jobs.array == NULL) terror("Could not reallocate launched_jobs array");
					finished_jobs.array= realloc(finished_jobs.array, jobs_times.number_of_reallocs * ARRAY_SIZE * sizeof(uint64_t));
					if (finished_jobs.array == NULL) terror("Could not reallocate finished_jobs array");
					aborted_jobs.array = realloc(aborted_jobs.array, jobs_times.number_of_reallocs * ARRAY_SIZE * sizeof(uint64_t));
					if (aborted_jobs.array == NULL) terror("Could not reallocate aborted_jobs array");
					nodes_usage.array = realloc(nodes_usage.array, jobs_times.number_of_reallocs * ARRAY_SIZE * sizeof(uint64_t));
					if (nodes_usage.array == NULL) terror("Could not reallocate nodes_usage array");
					cpu_usage.array = realloc(cpu_usage.array, jobs_times.number_of_reallocs * ARRAY_SIZE * sizeof(uint64_t));
					if (cpu_usage.array == NULL) terror("Could not reallocate cpu_usage array");
					mem_usage.array = realloc(mem_usage.array, jobs_times.number_of_reallocs * ARRAY_SIZE * sizeof(uint64_t));
					if (mem_usage.array == NULL) terror("Could not reallocate mem_usage array");
					cost_per_second.array = realloc(cost_per_second.array, jobs_times.number_of_reallocs * ARRAY_SIZE * sizeof(uint64_t));
